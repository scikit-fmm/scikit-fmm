...
private:
  ...
  void             solve();
  int              _getDimIndex(int current, int dim)
  {
    int index = current / shift_[0];
    for (int i=1; i<=dim; i++)
    {
      current -= index * shift_[i-1];
      index = current / shift_[i]; // integer assignment is like floor()
    }
    return index;
  }
...
protected:
  ...
  int              _getN(int current, int dim, int dir, int flag)
  {
    ...
    int newc = _getDimIndex(current, dim) + dir;
