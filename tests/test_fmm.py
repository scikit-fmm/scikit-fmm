...
def test_simple_case():
    ...
    np.testing.assert_array_equal(travel_time([1,0,-1],[1,1,1]), [1, 0, 1])
    np.testing.assert_array_equal(travel_time([-1,0,1],[1,1,1]), [1, 0, 1])
...
def test7():
    ...
    try:
        ...
        assert ValueError.__name__ == ex.__class__.__name__
        assert ex.args[0] == "the array phi contains no zero contour (no zero level set)"
    else:
        raise Exception("no exception thrown")
